...
The harness provides a **single implicit team per orchestrator session** (no `TeamCreate`/`TeamDelete`). Spawned agents do NOT have the `Agent` tool — only the orchestrating instance spawns: hub-and-spoke; managers request implementer spawns via `SendMessage`. **Never pass `team_name`** — it is a deprecated, ignored Agent-tool parameter and the `validate_no_team_name` hook blocks any spawn carrying one (#1375). Every agent belongs to the one implicit team regardless of which repo it edits; the repo is expressed by its worktree and brief. Per-repo rosters under `<repo>/.claude/team/roster/` remain canonical for **commit identity, domain ownership, and reviewer pairing**. Full mechanics: [`.claude/team/charter/agents/orchestration-model.md` § Single-Leader Constraint](.claude/team/charter/agents/orchestration-model.md) — read before spawning the team in a new session.
...
- **Branching strategy:** feature branches named `{FirstInitial}.{LastName}/{IIII}-{issue-name}` (e.g., `N.Khoury/0042-update-charter`) merged to `main` via PR — **except the bookkeeping allowlist below**, which commits directly.
- **Direct-to-`main` allowlist (#1487).** These four paths — and only these — are committed straight to `main` without a PR, because they are generated or append-only records of work already reviewed elsewhere: `.claude/memory/**` · `cross-repo-status.json` · `ontology/**` (incl. `checksums.json`) · `.claude/generic_prompt_ledger.json`. **Everything else keeps the full PR gate**, including 2 reviewers + ≥1 Opus merge-gate: code, `.claude/hooks/`, `.claude/lib/`, `.claude/skills/`, `.claude/team/`, `.github/`. Measured over the 60 most recent first-parent commits on `main` (2026-09-03): 31 via PR, 29 direct, and the 29 touched **only** those four paths — never a hook, lib, skill, charter, or workflow. Branch protection deliberately stays ON, so an allowlisted commit still prints `Bypassed rule violations` — a visible, auditable bypass over a known-good set beats silently widening who may push.

### Shell environment: zsh

The dev shell — interactive **and** the agent Bash tool — is **`zsh`**, not bash. Bash-only idioms (`declare -A`, `${!arr[@]}`, unquoted URLs/globs) silently break; default to POSIX-portable constructs, `bash -c '…'` when bash is genuinely required. Do/don't list: [`docs/TOOLCHAIN.md`](docs/TOOLCHAIN.md) § Shell environment, codified in [`ontology/conventions.md`](ontology/conventions.md). For text search use **`rg` (ripgrep), never plain `grep`** — a bare `grep` is hard-blocked ([#1008](https://github.com/noorinalabs/noorinalabs-main/issues/1008)); add `--no-ignore` when the target may be `.gitignore`d (e.g. `ontology/structural/`) **and `--hidden` whenever a recursive search from the repo root could cover `.claude/`** — it is a dotted directory, so `rg` skips it by default and a repo-wide sweep for any hook/lib/skill symbol returns a **silent zero** (`-uu` implies both flags). Either omission gives you a confident empty result, not an error. Prefer `ast-grep` (invoke as `ast-grep`, never `sg`) over `rg`/`sed` for *structural* search/replace — TOOLCHAIN.md § Text search and § Structural & AST tooling. Optional native **LSP** (exact-symbol nav + post-edit diagnostics, ~0 context cost) is available via the committed `code-intelligence` plugin — one-time enable in TOOLCHAIN.md § Native LSP.
...
- All repos use **GitHub Flow** (feature branches off `main`, PRs for merge) — with the bookkeeping allowlist in § Developer Tooling as the one carve-out (#1487)
