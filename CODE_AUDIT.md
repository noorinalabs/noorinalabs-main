...
3. **Org gate tooling is vendored 7–8× and drifting.** `pre_commit_ci_sync.py`, `check_dockerfile_base_pin.py`, `check_fixture_realism.py`, `structural_ontology.py` and their tests total **~10.9K raw lines** for what is logically one ~2.8K-line package. The drift is not hypothetical: landing-page's copy cannot classify `cspell`, so its local⇄CI parity gate is false-green on a real gap (BUG-11), and ingest-platform's copy is 140 lines behind the parent's rewrite.
4. **Workflow YAML is ~15.1K lines org-wide with heavy copy-paste**: a byte-identical `auto-close-issues.yml` in 7 repos, near-identical `docs.yml` in 7, `structural-ontology.yml` in 5, a shared `ghcr-publish.yml` skeleton in 4, and stg/prod twin deploy workflows + smoke scripts (93% identical) inside deploy. Zero `actions/cache` usage in deploy (16 uncached `setup-python` steps, 12 uncached `terraform init`s).
...
|---|---:|---|
| A — Shared libraries | ~11,500 | models dedup (~2.6K), devtools consolidation (~8.1K raw), utils/arabic (~0.8K) |
...
| C — CI/CD & workflows | ~3,800–4,400 | reusable workflows, dead workflows, stg/prod parameterization, anchors |
...
| BUG-03 | data-acquisition ↔ ingest-platform | Kafka contract broken: producer `RawNewMessage{source,b2_key,size_bytes,acquired_at,checksum_sha256}` vs consumer `PipelineMessage{batch_id,source,b2_path,timestamp,record_count}` with `extra="forbid"`; `parse_message` sits *outside* the DLQ try (`workers/lib/runner.py:102` vs `:112`) so every message **crashes the dedup worker**. Fix: align producer to consumer shape now; move `parse_message` inside the DLQ try; add a cross-repo round-trip contract test. Durable fix = A4. | **High (pipeline down)** |
...
*Baseline (measured):* org-wide vendored footprint ≈ **10,920 raw lines** (sources 6,433 + test copies 4,487); drift live in 2 repos. *After:* one canonical copy ≈ 2,800 lines + thin config per repo ⇒ **≈ −8,100 lines** org-wide and the drift class eliminated. *Verify:* each repo's sync-drift gate stays green; `pre-commit run --all-files` parity per repo; the design-system classifier test (`scripts/tests/test_pre_commit_ci_sync.py`) moves upstream with the code.
...
Byte-identical in 7 repos (verified `diff -q`, 35 lines each = 245 org-wide).
*After:* 1 × ~40-line `workflow_call` + 7 × ~8-line callers ⇒ **≈ −150 lines** and one maintenance point. *Verify:* trigger a close in one repo per caller.

**C2 — `docs.yml` → reusable with inputs** · Repos: 7 (all but none — every repo has one; deploy's diverges most) · Effort: M · Depends: C1 host decision
...
*Baseline (measured):* ~1,876 lines org-wide. *After:* ~300-line reusable + 7 × ~15–25-line callers ⇒ **≈ −1,400 lines**. *Verify:* per-repo runs green with unchanged check semantics; sync-gate kind classification unaffected (A2 coordinates).

**C3 — `ghcr-publish.yml` → reusable build/push/dispatch** · Repos: ig, us, da, ip · Effort: M
Shared skeleton (build, GHCR push, notify-deploy dispatch) across 4 repos, 1,044 lines total; per-repo deltas = image matrix + dispatch event-type. Fold in the known asymmetries deliberately (landing-page intentionally has no sender — `deploy#285`).
*After:* ~350-line reusable + thin callers ⇒ **≈ −600 lines**; dispatch contract change becomes one edit. *Verify:* stg deploy dispatch fires per repo (ontology `cross_repo_dispatch_contracts` updated once).
...
7 hardcoded lists (one already drifted); comments are the only enforcement. Add `.claude/lib/org_repos.py` (or read `ontology/repos/*.yaml`), import everywhere, and add the prose→code test asserting the list matches the CLAUDE.md Repository Map (the repo already has gates of exactly this shape).
...
| A2 | Extract `noorina-devtools`; de-vendor 7 repos | all 8 | D/L | ~10,920 vendored lines → ~2,800 (−8.1K) | L | A1 |
...
| C1 | auto-close-issues reusable workflow | all | D | 245 lines ×7 → ~40+callers (−150) | S | host decision |
| C2 | docs.yml reusable with inputs | 7 repos | D | ~1,876 → ~450 (−1.4K) | M | C1 |
| C3 | ghcr-publish reusable | ig,us,da,ip | D | 1,044 → ~450 (−600) | M | C1 |
...
- **Duplication ground truth used above** (for re-verification): `diff -q` byte-identity for `auto-close-issues.yml` ×7 and `check_dockerfile_base_pin.py` ×8; `md5` identity for ig↔ip `arabic.py`; `difflib` ratios for the twin-repo file table; AST symbol-set diffs for drift direction; import-graph closure for ingest-platform's used-vs-freight split.
