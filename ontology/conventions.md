...
- **The PR base depends on the wave's declared merge model, which is chosen per wave and recorded in `cross-repo-status.json` as `wave_{M}_scope.merge_model`.** Charter rule: **One Merge Model Per Wave** (`charter/pull-requests.md`) — mixing them within a wave strands deliverables off `main` (origin: P6W1 stranded 5).
  - `wave-branch` — feature PRs target the wave deployment branch; final wave merge is deployment branch → `main` with user approval, and the wave/phase branches are **retained** (`--merge`, never `--squash`).
  - `direct-to-main` — feature PRs target `main` directly and there is no wave branch. This is what **wave-30** declared and used for all 12 of its PRs.
- `/session-start` Step 5c (`.claude/lib/wave_merge_model.py reachability`) checks live commits against the declared model mid-wave, so model-mixing or stranding surfaces within hours rather than only at the `/wave-wrapup` gate (#801).
...
### Agent coordination (hook-unenforced — read the artifact, never the report)

None of the following is enforced by a hook; each is a convention that has bitten more than once, and every one of them fails in the direction of a *plausible* wrong answer rather than a loud error.

- **Never infer work state from agent presence or agent self-reports — read the artifact.** `ListAgents` can show empty while an agent is still working, and agents routinely finish without reporting. In one wave-30 session **three merge gates went idle without reporting while having posted perfectly valid verdicts**; polling the oracle was the only thing that surfaced them. A posted verdict is also not a *counted* one (#1413). One rule: run `.claude/lib/pr_review_state.py` and read `distinct_reviewers` — never a self-report, never an eyeballed comment count.
- **Namespace every scratch file.** The session scratchpad is per-SESSION, not per-agent, so concurrent agents share one directory. Observed damage: a `--body-file` read another agent's file and a PR carried the wrong story's body with a wrong closing keyword; a sandbox's relative-path resolution imported a stale production-named module and mutation testing passed with a metacharacter deleted. Namespace by `<initials>-<issue>/`, never reuse a real module's basename, read back before consuming, and assert `module.__file__` in sandboxes.
- **Send the PR number to reviewers, never a head SHA.** A reviewer must re-confirm head at post time anyway, so a baked SHA is only ever older — a second, worse source of truth. Applies to mid-review corrections too: send "head moved", not a new SHA.
- **A head move drops EVERY verdict, and materiality is not an input.** `compute_review_state()` excludes any verdict cast before the head moved, however inert the delta. Re-request from *all* reviewers, not just the one who asked for the change. Where the delta really is inert, prove it (compare the two trees' ASTs with all doc text stripped, via `ast.unparse`) so prior reviewers can re-affirm briefly instead of redoing the pass.
- **Relay a reviewer's must-fix list verbatim — never renumbered or re-described.** The author fixes what the brief says and labels the commit with the brief's numbering; the *other* reviewer then rules on the brief's framing too. One substitution at the orchestrator becomes unanimous downstream agreement that the wrong item was discharged, and nobody can notice because the brief is the only version of the list they have. Preserve required-vs-recommended labels; promoting a "recommended" item is a substitution even when the item is real. **Corollary for reviewers:** on a re-review, check your *own* item list against what landed — an author's commit message naming your item numbers is not evidence those are the items addressed.
- **High agent fan-out exhausts the org's 5000/hr GraphQL quota**, after which agents die mid-task and a failed `gh` call's empty output reads as a legitimate zero result. `gh_quota_gate.py` blocks the mapped verbs with a REST rewrite; the unmapped ones only warn.
...
- **Session start protocol** (`.claude/skills/session-start/SKILL.md`, the arbiter): (0) worktree cleanup across the parent + 7 child repos, verify-merged-then-remove, never force-removing (#1212/#1213) + child-checkout refresh (#832); (1) team orientation; (2) handoff read; (2.5) two-tier memory index — ToC only, sections on demand (#1016); (3) ontology freshness — semantic overlay dirty count **and** structural-index regeneration, two independent layers; (4) Annunaki count-only; (5) wave/phase orientation via the compact `wave_status.py digest`, not the whole status file (#987); (5a) red default-branch verdict read; (5b) merged-but-unwrapped nudge; (5c) wave-branch reachability / merge-model check (#801); (6) charter freshness.
  **There is no team cleanup step.** The harness has no `TeamCreate`/`TeamDelete` (2026-06-16) — the session runs on a **single implicit team**, spawning is via the `Agent` tool with **no `team_name`** (deprecated and ignored; `validate_no_team_name` blocks a spawn carrying one, #1375), and only the orchestrator spawns.
- **Red default-branch workflow detection**: surfaces FAILED latest runs of in-scope workflows on `main` across repos — guards against silent default-branch rot (the GHCR publish red that went 12 days undetected, commit 5804476). W14 retro proposal #2, adopted PR #583. **Moved to a scheduled sweep (main#962):** the sweep + classification now runs in the scheduled `red-sweep.yml` workflow (every 6h; shared implementation `.claude/lib/red_sweep.py`), which persists a JSON verdict to the lightweight ref `refs/meta/red-sweep`; `/session-start` Step 5a is one cheap verdict read with a 24h staleness guard (missing/stale verdict → WARNING, never a false all-green). **Classification (P4W4 retro #3 / main#647):** the sweep best-effort classifies each red run's cause — a base-image-CVE signal (trivy/grype/apk-CVE/`openssl`-class advisory in the failed job log) is tagged **"base-image drift — fix-forward the base image, not a code regression"**, distinct from generic redness (degrades to `code/other` on log-fetch failure, never a false all-green); and `/wave-wrapup` Step 11.6a adds a fan-in **publish-freshness** check (latest `ghcr-publish.yml` on each fan-in repo's default branch), because the W4 openssl CVE-2026-45447 reddened the *publish* — not `deploy-stg.yml` — and so evaded the per-merge deploy watch. A red fan-in publish blocks wave closeability the same as a red deploy. **Scope is trigger-based, not name-based (main#1584):** a workflow is swept when its redness is not surfaced by a PR check — `schedule`-triggered, or `workflow_dispatch`-only, on the default branch — **union** the historical publish/deploy/release name class (`publish|deploy|release|promote|ghcr|image`), which is kept only so push-triggered publishes stay covered without a trigger read. Triggers are read from the workflow file's `on:` block (a run's `event` carries the one event that started it, never the workflow's trigger set, and PR-event runs never appear in a default-branch-filtered run list at all); an unreadable trigger set is swept IN and tagged `triggers-unreadable`, never dropped. The name-only predicate structurally excluded `e2e-stg-smoke` (isnad-graph, daily cron — 94 runs on `main`, 93 `failure` + 1 `cancelled`, 2026-06-13 to 2026-09-13, never once green) while the sweep reported all-green — a filter applied *before* classification means the sweep is not failing, it is accurately green over a set that excludes the red things.
...
| `block_bare_grep.py` | PreToolUse (Bash) | Block a bare `grep` — text search uses `rg` throughout (owner-directed 2026-07-19, #1008). `rg` is faster, multiline-capable and `.gitignore`-aware |
| `smart_grep_ontology.py` | PreToolUse (Bash) | Route a symbol-shaped `rg`/`grep` to the generated structural ontology instead of re-discovering structure by brute force (#1017) |
| `block_stale_tmp_message_file.py` | PreToolUse (Bash) | Refuse `git commit -F /tmp/…` and `gh {pr,issue} {create,comment,edit} --body-file /tmp/…` (plus `gh api --input /tmp/…`) when the target file is stale |
| `block_squash_wave_merge.py` | PreToolUse (Bash) | Block `--squash` into a `deployments/phase-*/wave-*` base (a squash into `main` is NOT blocked — gap tracked by #1177) |
| `validate_workflow_paths_coverage.py` | PreToolUse (Bash) | Block workflow-file orphans on `gh pr create` — a PR modifying a workflow that no workflow's `paths:` filter covers (deploy#153) |
| `validate_edit_completion.py` | PreToolUse | Block state-sensitive actions after an unhandled Edit/Write error — the **tool-error-soft-accept** failure class (P2W10, ≥2 occurrences) |
| `validate_wave_audit.py` | PreToolUse (Skill) | Block wave-lifecycle skills claiming a wave is concluded/complete until the cross-repo open-item audit clears (charter `skills.md`) |
| `warn_zsh_wordsplit.py` | PreToolUse (Bash) | Advisory on bash-only word-splitting idioms — the dev shell AND the agent Bash tool run under **zsh**, where they silently misbehave |
| `validate_wave_context.py` | PreToolUse (Agent) | Warn if agent spawned without wave context or ontology context in prompt |
| `validate_no_team_name.py` | PreToolUse (Agent) | Block Agent spawns passing `team_name` — a deprecated, ignored parameter; the session has a single implicit team, and there is no `TeamCreate`/`TeamDelete` (#1375) |
| `warn_oversized_brief.py` | PreToolUse (Agent) | Advisory when a brief pastes a whole charter/CLAUDE.md file — the largest per-token waste in the lean-brief program (#1020) |
...
| `auto_sync_main.py` | PostToolUse (Bash) | Fast-forward the local default branch immediately after a successful push/merge to `origin/main` (main#713, owner request 2026-06-19) |
| `warn_pipe_mask_rc.py` | PostToolUse (Bash) | Flag rc-masking pipes on `git push` / `gh pr merge` — absent `set -o pipefail` a pipeline returns the LAST stage's status, so a failed push reads as success |
| `ontology_tracker.py` | PostToolUse (Edit/Write) | Track file checksums for ontology changes. Skips gitignored-by-owning-repo files (#1039) and **any linked worktree**, detected structurally from the owning root's `.git` pointer file (`gitdir: …/worktrees/…`) rather than by directory name — the name-based `.worktrees/` filter leaked 24 orphan entries from differently-named worktrees. Clean up entries an earlier leak already landed with `python3 .claude/lib/checksums_io.py prune` — **previews by default, `--apply` required to write** (#1137); verify candidates are genuinely deleted, not branch-hidden, before applying. Read the dirty count with `checksums_io.py status`, never a hand-rolled JSON compare (#1142) |
...
| `session_start.py` | SessionStart | Point at `/session-start` and print one-line state counts. Slimmed per main#962 — it no longer prints the full protocol or the handoff contents, only an exists-pointer |
| `dispatcher.py` | PreToolUse (dispatcher) | Single entry point for all Bash/Edit/Write/NotebookEdit PreToolUse hooks — runs every hook's `check()` in-process instead of one subprocess per hook per tool call |
| `post_dispatcher.py` | PostToolUse (dispatcher) | Mirror of `dispatcher.py` for the PostToolUse phase; routes by matcher (Bash, Edit, Write, NotebookEdit) |
