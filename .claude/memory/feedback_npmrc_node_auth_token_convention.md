...
Related: [[feedback_security_guard_inline_not_followup]] (don't leave literal tokens; `${VAR}` is npm runtime substitution), [[feedback_statuscheckrollup_ci_clean]] (catching the 401 requires CI-state check), [[feedback_gh_cli_gotchas]] (same silent-401/no-op-with-no-log-breadcrumb shape — read-back-verify).
