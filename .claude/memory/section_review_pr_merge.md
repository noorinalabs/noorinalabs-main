...
- [After a rebase use patch-id, not ancestry](feedback_patch_id_after_rebase_not_ancestry.md) — a rebased head makes `--is-ancestor` FAIL and `diff <old>..<new>` OVERSTATE (1 real file read as 6). `git patch-id --stable` proves content survived; then diff from the rewritten base. Why charter:114 wants additive commits. W29 #1187.
