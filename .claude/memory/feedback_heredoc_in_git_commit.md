...
**Judge pass 2026-09-13 (#1550) — KEPT; the fix shipped but the rule did not move.** The `superseded_by` points at the `validate_commit_identity.py` parser fix (main#188 / PR#248). That fix landed, but the operative instruction — *multi-line or quoted commit message goes in a file and is passed with `-F`, never an inline `$(cat <<'EOF' ...)` inside a `git -c ...` line* — is in no charter section (`agents/orchestration-model.md` item 9 is the unrelated /tmp file-race rule). It is still cited as **live** guidance by `.claude/skills/promotion-audit/SKILL.md`, and `.claude/hooks/validate_commit_identity.py` still prints *"See memory feedback_heredoc_in_git_commit.md for full context."* in its block reason (a string `test_validate_commit_identity.py` asserts on). Exactly what the note predicted: *"the workaround is durable even after a fix."* Not archived — it states a currently-binding rule, not history.
