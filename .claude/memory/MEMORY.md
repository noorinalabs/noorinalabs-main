...
- [Prod reload — #723 NOT closeable](project_prod_loaded_quality_broken.md) — 07-01 corrected-artifact reload + API re-val: matn-as-narrator pollution STILL live on prod (≥7,580 nodes); my "matn=0" was a false pass (measured markup, not sentence-detection) → needs da#247 integrated NER re-run, tracker da#258 (wave-23). 50-cap NOW lifted (ig#1147✓), semantic graceful-503 (ig#1148). da#247/da#253 CLOSED but superseded-forward. cypher-pass ≠ record-level check.
