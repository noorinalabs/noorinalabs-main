...
- [Narrator choke-points enrich](project_narrator_chokepoints_enrich.md) — 07-09 da#337 resolve re-run DONE+PUBLISHED (`staged/narrator-resolve/2026-07-09-dcb6205`; 150,187→129,234 canonical; gate MET — al-Zuhrī/Abū Hurayra/Shuʿba/Sufyān top, no kunya blob). **stg load BLOCKED on deploy#549 stale B2 secret (owner must rotate).** Prod still HELD. Residual: da#345/#346/#347 filed, not started.
...
- [B2 pipeline publish key](reference_pipeline_b2_publish_key.md) — PIPELINE_B2_* is 2 keys same-name: GH secret READ-ONLY (VPS consumer), local ~/.zshenv WRITE (publish_parquet producer); z4h zshenv early-return hides exports from non-interactive tool shell (put them ABOVE the guard); read-only-key write 401 misreads as "failed to create bucket"; **GH secret never rotated post-split → consumer 401s, deploy-data-load has NEVER worked (deploy#549); its "no objects found" is a lie — ~0.5s=empty, ~5s=auth-fail**; repo-level VPS_HOST is a recycled IP (deploy#550); diagnose caps via b2_authorize_account v3 apiInfo.storageApi.capabilities.
...
- [Prod #723 crit-1 — head clean, tail real](project_prod_loaded_quality_broken.md) — 07-02 record-level graph check: UI head CLEAN (219,849, real top-20, 0 mubham, English-matn gone), stg↔prod parity✓. ≥7,580 isnad-verb pollution REAL but 96.6% mention≤1 tail (11 at mc≥5). da#258 tail fix (NER split/truncate/guard), da#248 cycles, da#259 loader-hang. Weight by mention_count.
- [P7 #723 resolve pollution + OOM fixes](project_p7_narrator_pollution_resolve_fixes.md) — resolve build-host=LOCAL VM; 40% pollution fixed at NER (da#247) + matn-opener scrub (da#311/#312, 150,187 canonical). **#723 CLOSED 07-05: scrubbed artifact loaded stg AND prod, record-level verified at exact parity — all 4 criteria PASS (matn 0.000%, collection 99.96%, chains 587,932, search ONLINE, parallels 4.49M).** Loader rc=1 NON-BLOCKING = validation-harness multi-statement CypherSyntaxError (da#319) + orphan-tail 44,073 at stg parity; not a data defect. Parity #916.
