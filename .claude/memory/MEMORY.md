...
- [Prod reload — #723 PARTIAL, re-val FAILED](project_prod_loaded_quality_broken.md) — 06-29 reload fixed orphans/chains/linkage (1.66M nodes) BUT 06-30 owner UI re-val found prod STILL broken: matn-as-narrator pollution persists (da#253, likely pre-da#247 load), semantic search fails on prod (ig#1148), search capped at 50 (ig#1147). #723 NOT closeable. cypher-pass ≠ UI walkthrough.
