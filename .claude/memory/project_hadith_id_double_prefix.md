...
**CORRECTION 2026-07-09 — `sanadset:sanadset:` is NOT a legitimate id.** An earlier note here (and in `MEMORY.md`) claimed it was a valid `corpus:collection` pair whose collection merely "fell back to the CSV stem," and that the da#355 collapse guard was therefore eating a valid segment. **Both halves are wrong.** Measured across all nine staged corpora: `collection == corpus` occurs on **650,986 / 650,986 sanadset rows and on ZERO rows of any other corpus** — and sanadset has exactly **one** distinct collection value. That is the CSV-stem fallback firing on *every* row because `books.csv` never mapped, which `src/parse/sanadset.py:551` itself calls out: *"silently falling back to the CSV stem destroys corpus identity."* It is da#353's defect, not a legitimate shape.

Consequences: (a) the hypothetical "collection legitimately named after its corpus" (`lk:lk:1`) **exists nowhere** — do not cite it as a reason a collapse is unsafe; the real reason is that `collection == corpus` is *forbidden* as a producer output because it is ambiguous; (b) da#359's `DoubledCorpusPrefixError` is therefore **correct** to raise on it, and the raise must be paired with a producer-side gate in `generate_source_id` plus loader quarantine, since every sanadset id on disk today aborts a load (82% of all mentions). See [[feedback_sweep_expensive_stage_before_launch]] — this misconception survived because the check that would have caught it read its input from the thing it was checking.
