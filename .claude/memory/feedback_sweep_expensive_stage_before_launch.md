...
- **The escape from a hand-maintained list is usually another hand-maintained list, one abstraction up, where it does not look like one.** A guard reddened on a legitimate site (`raise SystemExit(main())` in a standalone tool). The two obvious fixes: *exempt the file* — a list of permitted files; or *scope the scan to "the pipeline"* — a list of participating modules. Both were refused. The real move was to stop asking **"is this site in scope"** and ask **"what value does this expression emit"**: permit `0`, permit a name resolving to the registry, and follow a call **one hop** into a module-local callee's `return` values. The tool went green untouched, and the guard got *stronger* — every prior predicate waved a `return 3` inside that callee straight through. Fail-closed at the hop boundary, stated rather than forgotten. **Ask the language, not the inventory** — the same move that replaced a reserved-value list with `enum`, a node-type table with an `ast.Name` in `Store` context, and a blacklist of bad argument shapes with an allowlist of provably-good ones (which catches `IfExp`, `int(cfg)`, `codes[0]` with nothing enumerated).
- **A specification that cannot be executed will be implemented three ways and agree with none of them.** A ruling said *"permit a `Name`/`Attribute` the sole-declarer machinery resolves against the registry."* Three artifacts resulted: the **prose** (correct, and the only one nobody could run); the ruler's own **verification script**, which reddened the right cases because it carried a hardcoded `REG = {...}` literal set — *inside the script verifying the ruling that abolishes hardcoded literal sets*, agreeing with its author by containing the answer; and the **implementation**, which read "any `Name`" and permitted `sys.exit(x)`, the one thing the guard existed to forbid. Meanwhile a reviewer implemented the prose faithfully and hit a `RecursionError` **that exists nowhere but in the prose** — the shipped code passes `funcs=None` and cannot recurse. Nobody was careless. *The implementation drifted from the intent because the intent was the artifact with no test.* When a rule is worth ruling on, **ship it as a runnable predicate with its case table, not as a sentence** — and if you must write the sentence first, say which artifact is normative.
- **Never silence a new guard's first finding.** Six hand-maintained lists on one issue, each born from the urge to quiet a guard that had just done its job on its first execution. If the guard reds on a real site, that is the guard working. Route the value, delete the instance, or state the boundary — do not add the site to a list.
- **A guard that checks the form of a claim cannot check its truth**, so know which of your mechanisms are which. `@enum.unique` makes a duplicate *declaration* inexpressible and says nothing about whether the code emitted is the *right* code; a bare-int-exit detector runs green over a correctly-named **wrong** constant. Buying a form-check by deleting a truth-check (an ordering assertion for a per-member statement of what is on disk) is a downgrade even when the form-check is stronger.
- **Two-sided instrument guards.** An allowlist that permits nothing reds on the whole tree; one that permits everything reds on none of it. **Neither is a detector, and both are green in the direction you would glance at.** Require both halves non-empty. The one-sided version — *prove the instrument can see* — is what let a scan pass while pointed at the wrong corpus ([[feedback_silent_zero_is_not_a_measurement]]).
