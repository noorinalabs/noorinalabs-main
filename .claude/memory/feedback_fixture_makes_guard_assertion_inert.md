...
> `/promotion-audit` flags this note as a *stale opt-out*: `promotion_target=none` while `retro_citations=7`, over the `2 × threshold` line. **The citation count is inflated and is not evidence of use.** Of the 7 occurrences in the feedback-log corpus, **2 are substantive** (`feedback_log.md:633,656` — the wave that derived the lesson) and **5 are bookkeeping**: four are `/wave-retro` Step 7.8 *size-sweep* listings (`:215, :276, :351, :669`) that print this filename every wave purely because the file is over the 14 KB soft ceiling, and one (`:809`) is the Step 7.7 audit reporting the citation count itself.
...
- **An anti-vacuity assertion written as an equality against the collection it guards is itself vacuous.** `assertEqual(reached, list(BLOCKING_CASES))` — the guard proving the previously-dead branch executed — **passes under the empty-`BLOCKING_CASES` mutant**, because it degrades to `assertEqual([], [])`. The emptiness it exists to detect is exactly what makes both sides equal. Pair it with a bare `assertTrue(reached, ...)` *ahead* of the equality; neither alone suffices, and the equality alone is the trap. (Aino Virtanen, main#1215 round 2, 2026-08-02 — caught **only** by running the empty-matrix mutation, in the very fix she was writing to remove an inert guard blocked under this note's own rule. The #1203 defect would otherwise have shipped a second time inside its own remedy.) This is the third recorded instance of the bullet above it: *an instrument built to catch this failure is the most likely to contain it.*
...
- **Write the mutant before the assertion.** Not after, when the assertion is already green and mutating it feels like a formality. (Nikolaos Papadopoulos's own diagnosis of four wrong-reason greens: *"I was fastest to reach for a check when I was most confident of the answer."*) The ordering is what forces the mutant to have something to bite: a test written mutant-first cannot accidentally verify a *different object* than the one under test — as `test_the_decorator_is_actually_applied` did, building its own `IntEnum`, applying `enum.unique` to **that**, and asserting it raised. It would have passed with `@enum.unique` stripped from `ExitCode`. **A verified plant into an unverified subject.** (An earlier draft said it would also have passed with no members and from another repository. Measured: both **error**, on `AttributeError` and `ModuleNotFoundError`. The test does depend on its subject existing and carrying `MISSING_DEPENDENCY`; what it never touches is the decorator — which is the whole of the defect and enough to damn it. A three-part flourish, two parts false, one bullet below *"a claim about a set, written while looking at one member."* Aino Virtanen tested each particular.)
...
- **And the contaminated RED is the more dangerous one.** Same author, an hour later, reproducing a real blocker: she planted a duplicate enum member immediately after `ENRICH_FAILED = 7`, and one test failed. It failed *for the wrong reason* — inserting a member displaced `ENRICH_FAILED`'s own docstring, so the AST docstring guard reddened on `ENRICH_FAILED`, not on the alias. She nearly reported the blocker **confirmed** on evidence that had nothing to do with it. A contaminated green gets caught downstream when the bug ships; **a contaminated red produces a correct-looking verdict that survives review forever** — the blocker was real, the fix was right, and the reasoning underneath was garbage. Nothing ever contradicts it. So the rule covers both signs: *assert the fixture is what you think it is*, and make the plant touch **exactly** the thing under test — here, that the decorator is stripped, the planted member carries its own docstring, and all seven pre-existing members still carry theirs.
- **A guard that checks the form of a claim cannot check its truth.** `@enum.unique` makes a duplicate *declaration* inexpressible and says nothing about whether `LOAD_FAILED` is what actually happened; a bare-int-exit detector runs green over `sys.exit(ExitCode.LOAD_FAILED)` in a helper that has no idea which stage called it. Both fired green for an hour while the registry asserted a falsehood in a *named constant*. Know which of your mechanisms are form-checks (most of them) and which are truth-checks (the behavioural test; the per-member docstring stating what is on disk). Buying a form-check with the deletion of a truth-check is a downgrade even when the form-check is stronger. (Nikolaos Papadopoulos, da#384.)
