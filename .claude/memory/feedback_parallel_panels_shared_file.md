...
- NEVER pair an `issue close` with a PR `merge` in the same un-guarded batch. Confirm `merged:true` (or re-read PR state) BEFORE closing the issue — a wave-branch merge doesn't auto-close anyway, so the close is always a separate deliberate step gated on verified merge. Companion to [[feedback_wave_branch_issue_close]].
