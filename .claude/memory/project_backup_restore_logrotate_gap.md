...
**Coverage gap (deploy#559).** `backup.sh` dumps only `neo4j` + the isnad `postgres`. Twelve stateful containers run. **`user-postgres` has NO coverage** — it holds accounts/sessions/RBAC *and* the `audit_log` relocated out of Neo4j into user-service Postgres on 2026-06-30 (see [[feedback_stg_gate_before_prod]] § prod-rollout mechanics). Unlike the graph, none of it is reconstructible from the published Parquet. Also uncovered: `redis`, `user-redis`, `kafka`, `loki`, `grafana`. The deliverable includes *explicitly declaring* which stores are disposable — an undocumented omission and a deliberate exclusion look identical during an incident.
