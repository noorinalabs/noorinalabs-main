...
When a state-claim is about a **memory file** (under `~/.claude/projects/.../memory/`) — "the forward-pointer is/isn't written", "the citation is/isn't present", "the clarification is/isn't there" — verify via **full `Read` or `rg -n <token>`**, NOT via `tail -N`.
...
**How to apply:**
- For any state-claim of the form "X is/isn't in `<memory-file>`", run `rg -n "<token>" <memory-file>` or `Read` the full file before stating the claim. NEVER tail-only. (Bare `grep` is hard-blocked, #1008.)
- If a PR body lists a memory file as a "changed" file, the verifying read is `rg -n "PR #<N>"` or `Read` the full file (memory dir is outside repo, no `gh api` route).
