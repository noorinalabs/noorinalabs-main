...
**Judge pass 2026-09-13 (#1550) — KEPT, `superseded_by` is only PARTIAL.** `charter/state-claims.md` § Canonical Source via `git show <sha>:<path>` carries the original rule (steps 1-3, severity, the #112/#186 worked example) but carries **nothing** of the 2026-06-21 / main#779 SIBLING-REPO variant above: it is phrased entirely around a local-`main`-lags-`origin` sync task and never names reading **another repo's** file at *that* repo's origin (`gh api .../contents`, `git show origin/main:<path>` after fetch), nor the line-count-mismatch tell. A reader holding only the charter still reaches for the local sibling checkout — the exact move that nearly overturned a correct review. The note's own `/promotion-audit` recommendation (broaden the charter section to name the cross-repo refutation case) is still unactioned.
