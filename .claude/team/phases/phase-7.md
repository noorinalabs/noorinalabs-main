...
| 7 | **Streaming pipeline repeatable** — Kafka E2E reproducible one-command (P5 end-state #5 carryover) | `main#667` | existing |
...
- [ ] All 8 end-state criteria above hold (trackers closed / verified on prod)
- [ ] Per-wave TD-intake compliance verified across W1–W5
- [ ] sanadset W2 decision recorded + executed; prod re-validated (orphan %, chains, search)
- [ ] On confirmation, `/plan-phase 8` defines the next phase (ML modeling nucleus) before any P8 wave kicks off
...
- Any product surface not blocking the 8 criteria — triaged at each `/wave-scope`.
