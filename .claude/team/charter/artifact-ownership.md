...
| **Ontology — structural layer** (`ontology/structural/`) | `noorinalabs-main/ontology/` | Built by the owned generator (#855); read by `/ontology-librarian` | **Parent-canonical, GENERATED, gitignored build product (main#939).** Always-current-by-regeneration — **not committed** (committing it made every concurrent PR conflict), **not** checksum-tracked, **not** resolved by `/ontology-rebuild` (#857, #939, #820/C×T2). Rebuilt on demand at `/session-start` 3b and `/wave-wrapup` 12b via `ontology_gen.aggregate` (which regenerates every in-scope repo's index locally); never hand-edit. |
