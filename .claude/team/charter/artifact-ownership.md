...
| **Ontology** (`domain.yaml`, `services.yaml`, `conventions.md`, `repos/*.yaml`, `checksums.json`) | `noorinalabs-main/ontology/` | Read by `/ontology-librarian` / `/ontology-rebuild`; tracker hook updates `checksums.json` on edits in any repo | **Parent-canonical, versioned.** Org-wide entities live here; per-repo internals under `ontology/repos/<repo>.yaml`. |
