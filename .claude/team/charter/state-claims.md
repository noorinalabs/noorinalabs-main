...
> **Stays prose (intentionally-prose, per the [#734 inventory](../charter_prose_inventory.md) `state-claims.md` row).** This whole verify-before-claim family — and its field-set sub-rules — reads like it could be a hook, but it cannot: the rule binds the *semantic* relationship between a natural-language assertion and a tool result ("does this `gh api` output actually confirm what I'm about to say"), which no deterministic gate can adjudicate. The sub-rules (which `--json` fields to fetch for a PR-state vs issue-state claim) are *recipes for the human/agent*, not gates — there is no tool boundary where "a claim is about to be made" is detectable. The section's own *Aspiration: post-publish audit* note already concedes the structural SendMessage-boundary hook "remains proposed." Mechanizing this would produce false confidence (a green check that the *fields were fetched*, not that the *claim is true*), which is worse than the honest prose. Cf. `feedback_enforcement_hierarchy`: a rule with no clean enforcement boundary stays charter-tier on purpose.
