...
`--admin` is no longer a silent bypass. `validate_pr_ci_status` blocks a `gh pr merge --admin` unless the operator declares a **charter-listed exception** via `ADMIN_MERGE_EXCEPTION="<class>:<rationale>"`. The `<rationale>` is required (non-empty) and, on an authorized use, is written via `log_pretooluse_block` into `.claude/annunaki/errors.jsonl` (`validate_pr_ci_status.py` lines 411–415; the record shares the same `type: pretooluse_block` schema the hook uses to log a *denied* attempt, `annunaki_log.py` lines 129–144). **That write is not a durable audit trail.** `errors.jsonl` is gitignored (`.gitignore`), untracked (`git ls-files .claude/annunaki` returns nothing), exists only on the machine that ran the merge, and is periodically rotated/cleared by `/annunaki-attack` (its `.bak.*` rotation files and `.claude/annunaki/archive/` are also gitignored). Nothing reads it back, either: `gate_integrity.recognise_exception_class` (`.claude/lib/gate_integrity.py` lines 320–338) derives a merged PR's exception class solely from the PR artifact — `charter_trailer.is_wave_branch`, the `emergency`-prefix title regex, the `doc-sweep` body-line regex — and never consults the declared `ADMIN_MERGE_EXCEPTION` string or this log. So today the rationale is auditable only by someone with shell access to the exact machine that ran the merge, before the next rotation — not "reviewable at retro time" in any durable, cross-machine sense. **Named gap, not implemented here:** the rationale would become durable if it were echoed into the merge commit message itself or into a PR comment on the merged PR, either of which `gate_integrity` could then be extended to read. The recognized classes:
...
**Why:** criterion #4 closes the silent-bypass class directly via two complementary gates. The ruleset is the server-side gate (covers UI merges, external actors, the batch-loop-evasion class); the hook is the operator-side gate (covers `gh pr merge`, names the exceptions, writes the local, machine-scoped exception log described above — not a durable audit trail). Defense in depth — neither alone is sufficient, because the ruleset's admin bypass would otherwise leave no local record at all and the hook alone doesn't cover non-`gh`-CLI merges. The hook-side gate is **active now** (this PR); the server-side ruleset is **active on the pilot now** and rolls out org-wide in W14 per the rollout-status note above. Note the two gates are mutually reinforcing on the apply order: because the hook already requires `ADMIN_MERGE_EXCEPTION` for `--admin`, the W14 rollout can apply default-branch rulesets without the admin-bypass becoming an unlogged hole the moment it exists.

### Dependabot base-image PRs (noorinalabs-deploy) take the full gate

Dependabot base-image / digest-bump PRs in `noorinalabs-deploy` take the **normal two-reviewer gate** (reviewer_1 + Opus merge gate) and are **never auto-merged on green** — owner ruling 2026-09-10 on [#1476](https://github.com/noorinalabs/noorinalabs-main/issues/1476), verbatim option label `Always review (Recommended)`, deciding between "auto-merge on green" and "always-review" for this PR class.

**Rationale:** green CI is not an enforced precondition in `noorinalabs-deploy` by design — it is one of the two path-filtered repos (§ Two path-filtered repos, above) and carries no `required_status_checks` in its branch-protection ruleset, so a passing rollup there is not the same guarantee it is in an unconditional-PR-CI repo. The concrete illustration that prompted the ruling: `deploy#706` (a dependabot base-image digest bump) merged with zero reviews, while `deploy#707` — the very next link in the same digest chain — got two reviews that found real defects CI did not catch. Same PR class, one week apart, opposite treatment; the ruling replaces "whichever happens that day" with an explicit, always-review policy.
