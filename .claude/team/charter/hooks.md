...
- **Input language:** PreToolUse Bash. Cheap pre-filter on the literal substring `--squash` short-circuits the common case before any parse or network call. On a match, tokenizes via the shared `_shell_parse` primitives (`strip_heredocs` → `tokenize` → `iter_command_segments` → `find_gh_subcommand`, per Hook Authorship Requirement 5) and yields each command-position `gh pr merge <N> --squash` segment with its `--repo`/`-R` value. Does NOT match: `--merge` merges (the correct method, never blocked even on a wave base); `gh pr view`/other gh subcommands; a `--squash` inside a heredoc body (prose, stripped first); a non-digit PR token (e.g. a `$pr` loop variable — unresolvable, and the batch-loop shape is guarded separately by `validate_pr_review`, memory `feedback_batch_loop_merge_evades`).
...
- **Test coverage:** `.claude/hooks/tests/test_block_squash_wave_merge.py` — squash-into-wave blocks (literal, with `--repo`, flag-order-independent), squash-into-main allows, `--merge`-into-wave allows, non-Bash tool allows, unresolvable base fails open, non-digit PR token never consults the runner, `--squash` pre-filter short-circuits the network path, and heredoc/compound-segment safety. Injected fake runner — never a real gh call.
