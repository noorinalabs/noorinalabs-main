...
That is the mechanism behind the whole four-copy drift: the canonical counter could be wrong for twelve waves while its tests stayed green, because nothing in the suite ever fed a real comment body through the real engine. The regression test that now exists runs the constructed filter through the **actual `jq` binary** — necessary because jq uses Oniguruma, not Python `re`, and the string passes through jq's own escape-collapsing before reaching the engine, so a Python-side `re.search` check would itself have been a fifth instance of testing the wrong thing. Related: `feedback_fixture_makes_guard_assertion_inert`.
