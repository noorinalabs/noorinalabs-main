...
Mechanical scoring (`trust_signals.py score 10 31`) over the canonical direct-to-main PR set: **17 scope-claiming PRs** across 2 repos (`noorinalabs-main`, `noorinalabs-deploy`), top-concentration **24%** (Nurul Hakim, 4/17 — far below the 60% fragility line), **0 CI-red merges**, **0** `review_false_positives`, 19 changes-requested verdicts.
...
**These signals are a LOWER BOUND, and it was checked that the bound does not move any delta.** Lucas Ferreira is missing `#1544` and `deploy#710`; Aino Virtanen is missing `#1520`. Lucas is already bump-eligible at `prs_merged=2` and cannot earn the review bump at `must_fix_caught=0`, so 4 PRs give the same `+1`. Aino sits in the neutral rework band at both 3 and 4 PRs (`6 > 3` forfeits the bump either way; `6 > 6` and `6 > 8` are both false, so no ding either way). Every delta below is therefore robust to the undercount — stated because "the inputs were incomplete" is otherwise an unfalsifiable caveat.
...
| Nino Kavtaradze | 5 | **5** | +1 | prs_merged=1, **must_fix_caught=6 — the wave's highest**, must_fix_received=0, rework_cycles=0, **0 CI-red**, false_positives=0. `clamp(5+1)` holds at the ceiling. Gating `deploy#710` he disproved a live-ruleset claim with a positive control (per-ref rules return 3 on `main`, 0 on two real `deployments/**` branches; 23 such branches unprotected), then ran the apply-script dry run with a **mutation control** that flipped UPDATE to CREATE, proving the ordering guard the spec documents is real rather than asserted. |
...
| **Orchestrator (Nadia)** | Caught the 17-vs-20 counter divergence and **refused to write until it was understood PR-by-PR**, rather than feeding the instrument its own number to make `--expect` pass; reconciled undeclared deploy scope instead of absorbing it; withheld the Annunaki archive because the log is not benign | **Hand-extended a SHA prefix in a rework brief** (`5937706b`, which resolves to nothing — HTTP 422); the merge gate caught it and reviewed the real SHA. A brief that names an unresolvable commit is the content-binding failure the review rule exists to prevent |
