...
Mechanical scoring (`trust_signals.py score 10 31`) over the canonical direct-to-main PR set: **17 scope-claiming PRs, every one of them in `noorinalabs-main`** — `wave_status.merged_prs('10','31', …)` returns **zero** `noorinalabs-deploy` PRs, which is exactly what the counter note below explains. The wave itself spans 2 repos; the *scored set* does not. Top-concentration **24%** (Nurul Hakim, 4/17 — far below the 60% fragility line), **0 CI-red merges**, **0** `review_false_positives`, 19 changes-requested verdicts.
...
Concretely, across the three uncounted PRs (`#1544`, `deploy#710`, `#1520`):
...
| Rated | Old | Applied | Delta | Reason |
|---|---|---|---|---|
| Aino Virtanen | 5 | **5** | 0 | prs_merged=3, must_fix_caught=5 (2nd highest), must_fix_received=6 (2.00/PR — **neutral band's upper edge**: 6 > 3 forfeits the bump, 6 > 6 is false so no ding), **0 CI-red**, false_positives=0. As merge gate on `#1542` she caught a new singular fixture that pinned an **ungrammatical** string — a test only the wrong value could satisfy, on the fifth corrective pass over that same line. |
