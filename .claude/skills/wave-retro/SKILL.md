...
- **Distribution discipline:** 5 is reserved for the wave's top relative performer (`trust_signals.apply_distribution_discipline`) — never handed out for merely-clean work. Call it with both populations: `apply_distribution_discipline({name: trust_signals.Proposal(old_score=..., proposed_score=..., signals=...)}, ranking_population=trust_signals.RankingPopulation.from_wave(phase, wave, status_path))`. `from_wave` takes the wave, so the anchoring population cannot arrive filtered; only use `RankingPopulation.declared(map, origin="…")` if you assembled the map by hand, and say in `origin` what it is. The entry-gate rule (a ceiling-holder is never evicted) is enforced by the function via `old_score`, so apply its output as returned (#1365). Both refusals are hard errors, not warnings: an empty population, or capping someone the population does not cover, raises (#1370).
